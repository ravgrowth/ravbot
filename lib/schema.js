...
export function getPool() {
  if (pool) return pool
  // Prefer app pooler URL (DATABASE_URL). Use SUPABASE_DB_URL only for tools/migrations.
  const url = process.env.DATABASE_URL || process.env.SUPABASE_DB_URL
  const source = process.env.DATABASE_URL ? 'DATABASE_URL' : (process.env.SUPABASE_DB_URL ? 'SUPABASE_DB_URL' : 'NONE')
  // debug (mask password)
  try {
    const u = new URL(url)
    const masked = `${u.protocol}//${u.username ? (u.username) : ''}${u.password ? ':****' : ''}@${u.host}${u.pathname}${u.search || ''}`
    logger.info('[db]', 'connecting', { source, host: u.host, db: u.pathname?.replace('/', ''), url: masked })
  } catch {
    logger.info('[db]', 'connecting', { source, url: url ? url.replace(/:(?!\/).+@/, ':****@') : null })
  }
  if (!url) throw new Error('No SUPABASE_DB_URL or DATABASE_URL in .env.server')
  // Enable SSL for Supabase hosts if not explicitly configured via URL
  const needsSsl = /supabase\.com/i.test(url)
  const config = { connectionString: url, max: 2, idleTimeoutMillis: 10_000 }
  if (needsSsl) {
    // Supabase requires TLS; don't reject self-signed in hosted envs
