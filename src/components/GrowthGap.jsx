  ...
  useEffect(() => {
    const load = async () => {
      ...
      const [v1, v2] = await Promise.all([
        ...
        supabase.from('idle_cash_recommendations_v2').select('idle_amount').eq('user_id', userId),
      ]);
      const rows = [...(v1.data || []), ...(v2.data || [])];
      const total = rows.reduce((acc, r) => acc + Number(r.idle_amount ?? 0), 0);
      setIdleTotal(total);
      setLoading(false);
    };
    load();
      ...
      {loading ? (
        ...
        <div>
          <div style={{ fontSize: '1.1rem', marginBottom: 8 }}>
            Idle cash: <b>{idleTotal.toLocaleString(undefined, { style: 'currency', currency: 'USD' })}</b>
          </div>
          <div style={{ fontSize: '1.3rem', fontWeight: 700, color: '#0b5fff' }}>
            You’re missing ~{yearly.toLocaleString(undefined, { style: 'currency', currency: 'USD', maximumFractionDigits: 0 })}/yr
          </div>
          {lifestyle.length > 0 && (
